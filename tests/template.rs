...
fn default_and_explicit_default_template_agree() {
    ...
    let templated =
        render_markdown_with_template(&doc, patpubrender::DEFAULT_TEMPLATE).unwrap();
...
fn custom_template_reorders_and_drops_sections() {
    ...
    let out =
        render_markdown_with_template(&doc, "{{claims}}\n\n---\n\n# {{title}}").unwrap();
    ...
    assert!(!out.contains("publication_number"), "frontmatter omitted:\n{out}");
}
