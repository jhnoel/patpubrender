...
pub(crate) fn parse_document(
    ...
    if document.node_name(root) != Some("us-patent-grant") {
        return Err(ParseError::UnsupportedStructure(
            "v4.x grants require us-patent-grant root".to_string(),
        ));
    }
...
fn write_bibliographic_information(output: &mut String, value: &BibliographicInformation) {
    xml::start_tag(output, "us-bibliographic-data-grant", &value.attributes);
    for part in &value.parts {
        match part {
    ...
    xml::end_tag(output, "us-bibliographic-data-grant");
}
